...
describe provider_class do
    ...
    describe "when checking status" do
        it "should call the external command 'sv status /etc/service/myservice'" do
    ...
    describe "when checking status" do
        it "and sv status fails, properly raise a Puppet::Error" do
            @provider.expects(:sv).with('status',File.join(@servicedir,"myservice")).raises(Puppet::ExecutionFailure, "fail: /etc/service/myservice: file not found")
            lambda { @provider.status }.should raise_error(Puppet::Error, 'Could not get status for service Service[myservice]: fail: /etc/service/myservice: file not found')
        end
        it "and sv status returns up, then return :running" do
            @provider.expects(:sv).with('status',File.join(@servicedir,"myservice")).returns("run: /etc/service/myservice: (pid 9029) 6s")
            @provider.status.should == :running
        end
        it "and sv status returns not running, then return :stopped" do
            @provider.expects(:sv).with('status',File.join(@servicedir,"myservice")).returns("fail: /etc/service/myservice: runsv not running")
