...
RSpec::Matchers.define :allow do |request|
  ...
  failure_message_for_should do |instance|
    "expected #{request.node}/#{request.ip} to be allowed"
  end

  failure_message_for_should_not do |instance|
    "expected #{request.node}/#{request.ip} to be forbidden"
  end
end
...
describe Puppet::Network::RestAuthConfig do
  ...
  def request(args = {})
    { :ip => '10.1.1.1', :node => 'host.domain.com', :key => 'key' }.each do |k,v|
      args[k] ||= v
    end
    stub 'request', :ip => args[:ip], :node => args[:node], :method => :find, :indirection_name => 'test',
                    :key => args[:key], :environment => 'test', :authenticated => true
  end
